...
start = time.time()
database = pickle.load(open('databaseA.txt', 'rb'))
database += pickle.load(open('databaseB.txt', 'rb'))
database += pickle.load(open('databaseC.txt', 'rb'))
database += pickle.load(open('databaseD+E.txt', 'rb'))
end = time.time()
print("....Database Loaded...")
print("Time taken to load Database....", end - start)


# monoDic = pickle.load(open('wordDict.txt', 'rb'))
...
def bothOccurence(word, db_in, alpha):
    Bigram = {}
    for i in db_in:
        if word in i:
            for j in range(len(i)):
                if i[j] == word:
                    start_point = j - alpha
                    if j - alpha < 0: start_point = 0
                    end_point = j + 5
                    if j + alpha >= len(i):
                        end_point = len(i) - 1
                    for z in range(start_point, end_point + 1):
                        # print i[z]
                        if i[z] == word:
                            continue
    ...
    for i in database:
        for j in i:
            ...
            if j not in monoDic:
                ...
                monoDic[j] += 1
    n = len(monoDic)
    pickle.dump(monoDic, open('wordDictA+B+C+D+E.txt', 'wb'))
    print(m, n)
    '''
    m, n = 4004227, 73179 
    #sys.exit(0)
