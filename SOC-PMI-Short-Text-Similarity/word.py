...
monoDic = pickle.load(open('wordDictA+B+C+D+E.txt', 'rb'))
...
def bothOccurence(word, database_in, alpha):
    Bigram = {}
    for i in database_in:
        if word in i:
            for j in range(len(i)):
                if i[j] == word:
                    ...
                    for z in range(start_index, end + 1):
                        # print(i[z]
                        if i[z] == word:
                            continue
...
def word_sim(word1="car", word2="automobile"):
    ...
    #			monoDic[j] += 1
    # n = len(monoDic)
    # pickle.dump(monoDic, open('wordDictA+B+C.txt', 'wb'))
    # print(m, n
